...
uint8_t   iVbat = 121, iTempC = 25, iVsolar = 20;
...
void adc_Volt_read(void)
{
  ...
  bat_Volt = getAdcLevel(0);
  pirFrame[2] = (bat_Volt >>8) & 0x0F;
  pirFrame[3] = bat_Volt & 0x00FF;
  iVbat = bat_Volt * 33 / 256;

  solar_Volt = getAdcLevel(1);
  pirFrame[7] = (solar_Volt >>8) & 0x0F;
  pirFrame[8] = solar_Volt & 0x00FF;
  iVsolar = solar_Volt * 33 / 256;

  temp_Volt = 938 - getAdcLevel(2);
...
void RTC1_IRQHandler(void)
{
  ...
  { 
    ...
    NRF_RTC1->CC[3] = (NRF_RTC1->COUNTER + 1 * 32768)& 0xffffff;     // Compare3 after approx COMPARE_COUNTERTIME 5 second
...
void DisplayLog(void)
{
  ...
  uint8_t   sendBuffer[20],sendindex=0;
  
  sendBuffer[sendindex++]=0x2B;
  ...
  for(i=0; i < maxcnt; i++)
    {
      ...
      sendBuffer[sendindex++] = BatLog[index];
      if (sendindex >= 20) {
        BLESerial.txmsg(sendBuffer,sendindex);
        sendindex=0;
      }
      sendBuffer[sendindex++] = SolarLog[index];
      if (sendindex >= 20) {
        BLESerial.txmsg(sendBuffer,sendindex);
        sendindex=0;
      }
      sendBuffer[sendindex++] = TempLog[index];
      if (sendindex >= 20) {
        BLESerial.txmsg(sendBuffer,sendindex);
        sendindex=0;
      }     
