...
volatile bool ble_on = true;
const char g_localName[] = "NRF Button RTC";
  ...
  if (g_sensorValueChanged)
  {
    ...
    if (ble_on==true){
       if (digitalRead(BUTTON_PIN1)==0){
          delay(2000);
          if (digitalRead(BUTTON_PIN1)==0) stopBle();
       } else {      
...
void RTC1_IRQHandler(void)
{
  ...
  if (NRF_RTC1->EVENTS_COMPARE[3] != 0)
  {
    ...
    NRF_RTC1->CC[3] = (NRF_RTC1->COUNTER + 5 * 32768)& 0xffffff;     // Compare0 after approx COMPARE_COUNTERTIME 1 second
